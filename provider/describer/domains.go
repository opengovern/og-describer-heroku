...
func processDomains(ctx context.Context, handler *resilientbridge.ResilientBridge, appName string, herokuChan chan<- models.Resource, wg *sync.WaitGroup) error {
	...
	for _, domain := range domains {
		wg.Add(1)
		go func(domain model.DomainJSON) {
			defer wg.Done()
			sniEndpoint := model.SNIEndpoint{
				ID:   domain.SNIEndpoint.ID,
				Name: domain.SNIEndpoint.Name,
...
func processDomain(ctx context.Context, handler *resilientbridge.ResilientBridge, appName, resourceID string) (*model.DomainJSON, error) {
	var domain model.DomainJSON
	baseURL := "/apps/"
